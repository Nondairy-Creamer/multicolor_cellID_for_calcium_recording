function [assignments, fixed_rotated, adjusted_rotated, assignment_distance] = get_pointcloud_assignments(fixed_points, adjusted_points, distance_threshold, max_allowed_assignment_distance)
...
%     [~, adjusted_cloud_r_nr] = pcregistercpd(adjusted_cloud_r, fixed_cloud, 'Transform', 'NonRigid');
    ...
    if remove_outliers
        fixed_to_adjusted_distance = fixed_cloud.Location - permute(adjusted_cloud_r_nr.Location, [3, 2, 1]);
    ...
    fixed_to_adjusted_distance = fixed_cloud_o.Location - permute(adjusted_cloud_r_nr_o_r_nr.Location, [3, 2, 1]);
    fixed_to_adjusted_distance = squeeze(sqrt(sum(fixed_to_adjusted_distance.^2, 2)));
    ...
    assignments = zeros(size(fixed_rotated, 1), 1);
    ...
    for ii = 1:size(fixed_rotated, 1)
        if assignments(ii) ~= 0
            assignment_distance(ii) = sqrt(sum((fixed_rotated(ii, :) - adjusted_rotated(assignments(ii), :)).^2));
        end
    end
