function create_trimmed_multicolor_file(data_folder)
    ...
    loaded_assignments = load(assignment_path);
    assignments = loaded_assignments.assignments;
    ...
    num_tracked_cells = length(assignments);
    ...
    for nn = 1:num_tracked_cells
        ...
        if assignments(nn) ~= 0
            multicolor_cell_is_tracked(assignments(nn)) = true;
        end
    end
    ...
    %% save the files
    cell_id_save_path = fullfile(multicolor_search.folder, multicolor_search.name, 'neuropal_data_ID.mat');
